...
$selcted                            =   'selected="selected"';
	...
	foreach ($certi_issue_records as $key => $certi_issue_record)
	{
		$tracksheet_id = $certi_issue_record['tracksheet_id'];
		$approved_by_md = $certi_issue_record['approved_by_md'];

		array_push($tracksheet_ids, $tracksheet_id);
		array_push($approved_by_mds, $approved_by_md);
	}
?>
...
<div class="db_filter">
	<div class="db_filter_mc">
        ...
        <div class="db_filter_cont">
	        <?php echo form_open(base_url('planning/list_certi_issue_checklist'),['id' => 'searchForm','class' => 'nor_form', 'name' => 'searchForm', 'method' => 'post'])?>
	         <div class="form_comp">
	           ...
	           <tr>
				...
				?>
						...
						<tr>
							...
							<td>
								<?php
									$approved_by_md_status = -1;

									foreach ($tracksheet_ids as $key => $tracksheet_id)
									{
										if($tracksheet_id == $data_id)
										{
											$approved_by_md_status = $approved_by_mds[$key];
											break;
										}
									}

									if($approved_by_md_status == 2)
									{
										echo "Yes";
									}
									else if($approved_by_md_status == 0)
									{
										echo "No";
									}
									else
							...
							<td>	
								<?php
									if(in_array($data_id, $tracksheet_ids))
									{
								?>
										<a class="btn btn-primary" href="<?php echo base_url('technical/view_certi_issue_checklist/'. $data_id); ?>">View</a>
								<?php
									}
									else
									{
								?>
										<a class="btn btn-primary" href="<?php echo base_url('technical/fill_certi_issue_checklist/'. $data_id); ?>">Fill</a>
								<?php
									}
								?>								
							</td>
