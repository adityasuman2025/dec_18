...
<div class="db_filter">
	<div class="db_filter_mc">
        ...
        <div class="db_filter_cont">
	        <?php echo form_open(base_url('planning/list_s_tracksheet'),['id' => 'searchForm','class' => 'nor_form', 'name' => 'searchForm', 'method' => 'post'])?>
	         <div class="form_comp">
	        ...
	        <thead>
	           <tr>
				...
				<?php
					...
					if($result_array)
					{
						$withdrawn_pending_status_tracksheet_ids = [];

						foreach ($list_tracksheets_with_pending_withdrwan_status as $key => $list_tracksheet_with_pending_withdrwan_status)
						{
							$tracksheet_id = $list_tracksheet_with_pending_withdrwan_status['tracksheet_id'];

							array_push($withdrawn_pending_status_tracksheet_ids, $tracksheet_id);
						}
				...
				?>
						...
						<tr>
							...
							<td>
								<?php 
									$tracksheet_status =  $resval->status;	
									...
									if(in_array($data_id, $withdrawn_pending_status_tracksheet_ids))
									{
										echo "MD Approval Pending";
									}
									else
									{
										echo $tracksheet_status_text;	
									}									
								?>								
							</td>	
							<td>																
								<a class="btn btn-primary" href="<?php echo base_url('planning/edit_tracksheet/'.$data_id); ?>">Edit</a>
							</td>
							<td>		
								<?php
									$tech_emp_assigned =  $resval->tech_emp_assigned;	
									if($tech_emp_assigned != 2)
									{
								?>
										<a href="<?php echo base_url('planning/assign_tech_to_tracksheet_page/'.$data_id); ?>">Assign</a>
								<?php		
									}
								?>		
																				
								<a href="<?php echo base_url('planning/view_tracksheet_info/'.$data_id); ?>">View</a>
							</td>
						</tr>
				<?php	
							$SlNo							=	$SlNo+1;			
